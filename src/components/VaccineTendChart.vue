<template>
  <div class="VaccineTendChart">
    <div ref="VaccineTendChart" style="width: 800px;height:620px;"></div>
  </div>
</template>

<script>
export default {
  name: 'VaccineTendCharts',
  ...
  methods:{
	myVaccineTendCharts(){
        var myVaccineTendChart = this.$echarts.init(this.$refs.VaccineTendChart);
        var titleName = this.areaId + "疫苗接种/新增病例对比图";
				var datay = [];
				for (let i = 0; i < this.data.length; i++) {
					var total_vaccines = "";
          var new_cases = "";
          var time = this.data[i].time*1000;
          if (this.data[i].total_vaccines !=-1) {
            total_vaccines = this.data[i].total_vaccines;
          }
          if (this.data[i].new_cases != -1 ){
            new_cases = this.data[i].new_cases;
          }
          datay.push([
              time,
              total_vaccines,
        ...
        myVaccineTendChart.setOption(option);
    }
    },
